...
export default function ManageProducts() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('price', String((form as any).price || 0));
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 max-w-3xl mx-auto py-12 px-4">
        ...
        <form className="mb-8 space-y-4" onSubmit={handleSubmit}>
          <input name="name" value={(form as any).name || ''} onChange={handleInput} placeholder="Product Name" className="border p-2 rounded w-full" required />
          <textarea name="description" value={(form as any).description || ''} onChange={handleInput} placeholder="Description" className="border p-2 rounded w-full" required />
          <input name="price" type="number" value={(form as any).price || 0} onChange={handleInput} placeholder="Price" className="border p-2 rounded w-full" required />
          ...
          {editingId && (
            <button type="button" className="ml-2 text-sm underline" onClick={() => { setEditingId(null); setForm({}); }}>Cancel</button>
          )}
        </form>
